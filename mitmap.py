import os
import time
from torpy.http.request import TorRequests
...
edited by : Swifty Client Prod (added a tor circuit)
"""
...
def show_ip(resp):
    for line in resp.text.splitlines():
        if 'you IP address currently is' in line:
            print(line)

with TorRequests() as Tor_requests:
    print("new build circuit is being built")
    with tor_requests.get_session() as sess:
        show_ip(sess.get("https://check.torproject.org/"))
        show_ip(sess.get("https://check.torproject.org/")) #this is just to show you the routing
    #using the torpy library you can nrenw the circuit at any time also choose how mnay circuits to run. 
